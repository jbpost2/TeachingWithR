...
- Importing CSV files with `readr` and excel files with `readxl`    
...
- Type `hist(iris$Sepal.Length)`  

- Submit these to console using button or hot key  

- Use the `Help` tab to learn about R functions  
...
- Use square brackets with a comma `[ , ]` [row, column]
...
- How does R locate the file?  
    ...
    + Can give *full path name*  
        ...
        * ex: C:/Users/jbpost2/repos/TeachingWithR/datasets/neuralgia.csv  
        * ex: C:\\\\Users\\\\jbpost2\\\\repos\\\\TeachingWithR\\\\datasets\\\\neuralgia.csv  
...
neuralgiaData <- read_csv("C:/Users/jbpost2/repos/TeachingWithR/datasets/neuralgia.csv")
```
...
```{r setwd1,eval=FALSE}
setwd("C:/Users/jbpost2/repos/TeachingWithR/datasets")
#or
setwd("C:\\Users\\jbpost2\\repos\\TeachignWithR\\datasets")
```
...
- File location: ".../datasets/neuralgia.csv
...
- Easily switch between analyses!  Works well with github!
...
- Behavior slightly different than a standard `dataframe`.  No simplification!  

```{r simplify}
neuralgiaData2 <- read.csv("https://www4.stat.ncsu.edu/~online/datasets/neuralgia.csv")
neuralgiaData2[,1]
neuralgiaData[,1]
```
...
```{r pull}
pull(neuralgiaData, 1) #or pull(neuralgiaData2, Treatment)
...
```{r, eval = FALSE}
myBatting[(myBatting$G > 20) & (myBatting$yearID == 2015), ] 
```
```{r}
...
- May only want certain variables (saw `$` and `[ , ]`)
...
```{r}
myBatting %>% filter(teamID == "PIT") %>% select(playerID, G, X2B) %>% arrange(desc(X2B)) 
```
...
- `group-by()` and `summarize()` are useful for creating basic summaries by groups 
```{r}
myAirquality %>% group_by(Status) %>% summarize(meanWind = mean(Wind), sdWind = sd(Wind))
```
...
## Other notes: Reshaping Data  with the `tidyr` package
...
ggplot(data = titanicData, aes(x = mySurvived, fill = sex)) + geom_bar()
ggplot(data = titanicData, aes(x = mySurvived, fill = sex)) + stat_count() 
```
...
```{r,eval = TRUE, out.width = "450px", fig.align='center'}
sumData <- titanicData %>% group_by(mySurvived, sex) %>% summarize(count = n())
ggplot(sumData, aes(x = mySurvived, y = count, fill = sex)) + 
...
```{r, echo = TRUE, out.width = "500px", fig.align='center'}
ggplot(titanicData) + 
  geom_histogram(aes(y = ..density.., x = age, fill = mySurvived)) +
  geom_density(adjust = 0.25, alpha = 0.5, position = "stack", 
               aes(x = age, fill = mySurvived)) 
```
...
ggplot(titanicData, aes(x = age, fill = myEmbarked)) + 
