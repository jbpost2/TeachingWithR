...
library(readxl)
#censusEd.xlsx in working directory
...
- Often only want select variables  
- `select()` function has same syntax as other `dplyr` functions!
...
- `summarize()` is often used with `group-by()` to obtain summaries without combining to the dataset  

```{r}
fandango %>% select(year, rottentomatoes) %>% 
  group_by(year) %>% summarize(avg = mean(rottentomatoes), sd = sd(rottentomatoes))
```
